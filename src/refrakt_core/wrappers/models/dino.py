...
class DINOWrapper(nn.Module):
    def __init__(self, model: Any, **kwargs: Any):
        ...
        filtered_kwargs = {k: v for k, v in kwargs.items() 
                          if k not in {"backbone", "out_dim", "model"}}
        self.wrapper_config = {
            'wrapper_type': 'dino',
            **filtered_kwargs
        }
    ...
    def forward(self, x: torch.Tensor, teacher: bool = False, **kwargs) -> ModelOutput:
        ...
        if hasattr(self.dino_model, 'backbone') and hasattr(self.dino_model.backbone, 'get_attention_maps'):
    ...
    def update_teacher(self):
        ...
        raise AttributeError("[DINOWrapper] Inner model has no method 'update_teacher()'")

    def parameters(self, recurse: bool = True):
        return self.dino_model.student_head.parameters()

    def named_parameters(self, prefix='', recurse=True):
        return self.dino_model.student_head.named_parameters(prefix=prefix, recurse=recurse)
