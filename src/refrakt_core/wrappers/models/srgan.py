...
class SRGANWrapper(torch.nn.Module):
    ...
    def training_step(
        ...
        if phase == "discriminator":
            ...
            pred_real = ModelOutput(
                logits=self.model.discriminate(hr),
                extra={"target_is_real": True}
            )
            pred_fake = ModelOutput(
                logits=self.model.discriminate(sr),
                extra={"target_is_real": False}
            )
            ...
            return {
                "d_loss": LossOutput(
                    total=total,
                    components={
                        "real": real_loss.total,
                        "fake": fake_loss.total
                    }
                )
            }
            ...
            if "generator" in loss_fn:
                ...
                pixel_loss = (
                    raw_pixel_loss if isinstance(raw_pixel_loss, LossOutput)
                    else LossOutput(total=raw_pixel_loss, components={"pixel": raw_pixel_loss})
                )
            ...
            raise ValueError("❌ 'phase' must be either 'generator' or 'discriminator' in SRGANWrapper.training_step()")
    ...
    def forward_for_graph(self, x: Optional[Union[torch.Tensor, Dict[str, torch.Tensor]]] = None) -> torch.Tensor:
        """
        ...
        if isinstance(x, dict):
            x = x.get("lr")
            if x is None:
                raise ValueError("forward_for_graph(): 'lr' key not found in input dict")
