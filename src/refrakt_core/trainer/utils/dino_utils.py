...
def _forward_pass_dino(model: Any, views: List[torch.Tensor], device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
...
def _backward_pass_dino(loss: torch.Tensor, optimizer: Any, scaler: Any, model: Any) -> None:
...
def _log_dino_metrics(
    ...
    if artifact_dumper and artifact_dumper.should_log_step(global_step):
        model_output = ModelOutput(
            ...
            extra={
                "backbone": getattr(model, "wrapper_config", {}).get("backbone", "unknown")
            },
        )
...
def handle_dino_training_step(
    ...
    try:
        ...
        # Logging
        _log_dino_metrics(
            model, student_out, loss_output, views, global_step, batch_id,
            artifact_dumper, logger, grad_log_interval, param_log_interval, optimizer
        )
