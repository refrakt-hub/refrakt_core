...
def _unpack_and_move_views_to_device(batch, device):
    view1, view2 = batch
    return view1.to(device), view2.to(device)


def _contrastive_forward_and_loss(model, view1, view2, device, loss_fn):
...
def _contrastive_artifact_logging(out1, out2, loss_output, global_step, batch_id, artifact_dumper, prefix1, prefix2):
...
def _contrastive_logger_metrics(model, optimizer, logger, global_step, grad_log_interval, param_log_interval):
...
def handle_contrastive_training_step(
    ...
    try:
        ...
        loss, out1, out2, loss_output = _contrastive_forward_and_loss(model, view1, view2, device, loss_fn)
        if loss is None:
            return 0.0, False
        ...
        scaler.update()
        _contrastive_artifact_logging(out1, out2, loss_output, global_step, batch_id, artifact_dumper, "train/view1", "train/view2")
        _contrastive_logger_metrics(model, optimizer, logger, global_step, grad_log_interval, param_log_interval)
...
def handle_contrastive_evaluation_step(
    ...
    try:
        ...
        if loss is None:
            return 0.0, False
        _contrastive_artifact_logging(out1, out2, loss_output, global_step, f"val_{batch_id}", artifact_dumper, "val/view1", "val/view2")
        return loss.item(), True
    except (RuntimeError, ValueError, TypeError) as e:
        return 0.0, False 