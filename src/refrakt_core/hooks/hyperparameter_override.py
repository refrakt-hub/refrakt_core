...
def parse_override_arg(override_arg: str) -> tuple[str, Any]:
    ...
    if '=' not in override_arg:
        raise ValueError(f"Invalid override format: {override_arg}. Expected 'path.to.param=value'")
...
def validate_config_structure(config: DictConfig, required_paths: List[str]) -> List[str]:
    """
