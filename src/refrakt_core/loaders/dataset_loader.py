...
def validate_gan_structure(extracted_path: Path) -> bool:
    ...
    lr_files = list(lr_dir.glob("*.png")) + list(lr_dir.glob("*.jpg")) + list(lr_dir.glob("*.jpeg"))
    hr_files = list(hr_dir.glob("*.png")) + list(hr_dir.glob("*.jpg")) + list(hr_dir.glob("*.jpeg"))
...
def validate_image_size(image_path: Path, max_size: Tuple[int, int] = (224, 224), max_ratio: float = 2.0) -> Tuple[bool, Optional[str]]:
    """
...
def validate_dataset_images(dataset_path: Path, max_size: Tuple[int, int] = (224, 224)) -> None:
    ...
    if invalid_images:
        error_messages = "\n".join([f"{path}: {msg}" for path, msg in invalid_images[:5]])
...
def load_custom_dataset(
    ...
    **kwargs
    ...
    if task_type is None:
        ...
        if task_type is None:
            raise ValueError(
                f"Could not detect dataset format. Available formats: gan, supervised, contrastive"
            )
    ...
    **kwargs
    ...
    if dataset_type == "auto":
        if dataset_path.endswith(".zip"):
            dataset_type = "custom"
        elif dataset_path.lower() in ["mnist", "cifar10", "cifar100", "stl10", "imagenet"]:
