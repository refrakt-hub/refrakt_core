...
class ImageSizeValidator:
    ...
    def validate_image(self, image_path: Path) -> Tuple[bool, Optional[str]]:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                if width > self.max_size[0] * self.max_ratio or height > self.max_size[1] * self.max_ratio:
                    error_msg = (
    ...
    def resize_image(self, image: Image.Image, target_size: Tuple[int, int] = (224, 224)) -> Image.Image:
        """
...
class DynamicDatasetLoader:
    ...
    def _find_val_directory(self, extracted_path: Path) -> Optional[Path]:
        ...
        for item in extracted_path.iterdir():
            if item.is_dir() and ("val" in item.name.lower() or "test" in item.name.lower()):
                return item
        return None
