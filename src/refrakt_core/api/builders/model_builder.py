...
from .utils.model_utils import (add_fusion_block, apply_model_overrides,
                                create_default_wrapper, instantiate_base_model,
                                validate_model_config, wrap_model)
...
def build_model(
    ...
    It supports optional model wrapping (e.g., for autoencoders) and can further wrap the model
    with a FusionBlock if specified. Robust error handling ensures a DefaultModelWrapper is
    used as a fallback.
    ...
    Args:
        cfg: Configuration object (OmegaConf or DictConfig) specifying the model structure,
             parameters, and optional wrapper settings
        modules: Registry dictionary containing available model and wrapper functions
    ...
    Returns:
        The instantiated and wrapped model object, ready for training or inference.
        The exact type depends on the model and wrapper configuration.

    Raises:
        TypeError: If the configuration or its fields are not of the expected type
        ValueError: If required model or wrapper components are missing or not found
                   in the registry
        Exception: Any other exceptions during model building will trigger fallback
                  to DefaultModelWrapper
    """
