...
def test_default_loss_wrapper_smoke_forward_tensor(
    dummy_loss, sample_output, sample_target
):
    wrapper = DefaultLossWrapper(dummy_loss)
...
# Sanity Tests
def test_default_loss_wrapper_sanity_model_output(
    dummy_loss, sample_output, sample_target
):
    wrapper = DefaultLossWrapper(dummy_loss)
...
def test_default_loss_wrapper_unit_mae_dispatch():
    ...
    model_output = ModelOutput(
        reconstruction=recon, extra={"mask": mask, "original_patches": original_patches}
    )
...
def test_default_loss_wrapper_unit_vae_dispatch(sample_output):
    ...
    model_output = ModelOutput(
        reconstruction=sample_output,
        extra={"mu": sample_output, "logvar": sample_output},
    )
    loss = wrapper(model_output, sample_output)
    assert hasattr(loss, "total")


def test_default_loss_wrapper_unit_loss_output_return(
    dummy_loss, sample_output, sample_target
):
    class LossReturnsLossOutput(nn.Module):
        def forward(self, x, y):
            return LossOutput(
                total=torch.tensor(1.0), components={"foo": torch.tensor(1.0)}
            )
