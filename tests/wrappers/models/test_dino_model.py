...
class MockDINOModelWithAttention(nn.Module):
    def __init__(self):
        ...
        self.student_head = nn.Linear(512, 1024)
        self.backbone = Mock()
        self.backbone.get_attention_maps = Mock(
            return_value=torch.randn(2, 4, 224, 224)
        )
...
@pytest.fixture
def model_config():
    return {"backbone": "resnet18", "out_dim": 1024}


@pytest.fixture
def dict_config():
    return DictConfig({"backbone": "resnet18", "out_dim": 1024})
...
def test_dino_wrapper_sanity_attention_maps_included(
    mock_dino_model_with_attention, sample_input
):
...
def test_dino_wrapper_unit_wrapper_config_filtering():
    ...
    mock_model = Mock(spec=nn.Module)
    wrapper = DINOWrapper(
        mock_model, backbone="resnet18", out_dim=1024, custom_param="value"
    )
...
def test_dino_wrapper_unit_named_parameters_returns_student_head_params(
    mock_dino_model,
):
...
def test_dino_wrapper_unit_attention_maps_handling(
    mock_dino_model_with_attention, sample_input
):
...
def test_dino_wrapper_unit_no_attention_maps_when_not_available(
    mock_dino_model, sample_input
):
...
def test_dino_wrapper_unit_extra_fields_consistency(mock_dino_model, sample_input):
    ...
    assert (
        output1.extra["wrapper_config"]["wrapper_type"]
        == output2.extra["wrapper_config"]["wrapper_type"]
    )
