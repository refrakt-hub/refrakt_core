...
def mock_loss_fn():
    return {
        "discriminator": Mock(
            return_value=LossOutput(
                total=torch.tensor(1.0), components={"d": torch.tensor(1.0)}
            )
        ),
        "generator": Mock(
            return_value=LossOutput(
                total=torch.tensor(0.5), components={"g": torch.tensor(0.5)}
            )
        ),
    }
...
def test_srgan_wrapper_sanity_training_step_discriminator(
    mock_srgan_model, sample_batch, mock_loss_fn, mock_optimizer
):
    wrapper = SRGANWrapper(mock_srgan_model)
...
def test_srgan_wrapper_sanity_training_step_generator(
    mock_srgan_model, sample_batch, mock_loss_fn, mock_optimizer
):
    wrapper = SRGANWrapper(mock_srgan_model)
    result = wrapper.training_step(
        sample_batch, optimizer=mock_optimizer, loss_fn=mock_loss_fn, phase="generator"
    )
...
def test_srgan_wrapper_unit_forward_for_graph_tensor_input(
    mock_srgan_model, sample_input
):
    wrapper = SRGANWrapper(mock_srgan_model)
...
def test_srgan_wrapper_unit_training_step_invalid_phase(
    mock_srgan_model, sample_batch, mock_loss_fn, mock_optimizer
):
    wrapper = SRGANWrapper(mock_srgan_model)
