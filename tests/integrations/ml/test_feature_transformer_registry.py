...
"""
    ...
    try:
        ...
        assert FEATURE_TRANSFORMER_REGISTRY['custom_scaler'].__name__ == 'StandardScaler'
    finally:
        # Restore original registry
        FEATURE_TRANSFORMER_REGISTRY.clear()
        FEATURE_TRANSFORMER_REGISTRY.update(original_registry)
...
def test_file_not_found_error_unit():
    ...
    with patch('pathlib.Path.exists') as mock_exists:
        ...
        with pytest.raises(FileNotFoundError, match="feature_transformer_registry.yaml not found"):
            load_feature_transformer_registry()
