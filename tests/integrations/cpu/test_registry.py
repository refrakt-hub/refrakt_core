"""
Comprehensive tests for sklearn registry module.
"""
...
def test_registry_structure_sanity():
    ...
    for model in expected_models:
        ...
        assert registry[model].startswith(
            "sklearn."
        ), f"{model} should map to sklearn class"


def test_registry_model_paths_sanity():
    """Sanity test: Verify that registry model paths are valid sklearn or xgboost paths."""
    registry = load_sklearn_registry()
    for model_name, class_path in registry.items():
        if model_name == "xgboost":
            assert class_path.startswith(
                "xgboost."
            ), f"{model_name} path should start with xgboost."
        else:
            assert class_path.startswith(
                "sklearn."
            ), f"{model_name} path should start with sklearn."
...
def test_registry_consistency_unit():
    ...
    registry1 = load_sklearn_registry()
    registry2 = load_sklearn_registry()
    assert registry1 == registry2, "Registry should be consistent across loads"
    assert set(registry1.keys()) == set(
        registry2.keys()
    ), "Registry keys should be consistent"
    for key in registry1:
        assert (
            registry1[key] == registry2[key]
        ), f"Registry value for {key} should be consistent"
...
def test_registry_xgboost_integration_unit():
    ...
    if "xgboost" in registry:
        assert (
            registry["xgboost"] == "xgboost.XGBClassifier"
        ), "XGBoost should map to correct class"
    else:
...
def test_registry_error_handling_unit():
    ...
    try:
        ...
        pytest.fail(f"load_sklearn_registry should not raise {type(e).__name__}: {e}")
