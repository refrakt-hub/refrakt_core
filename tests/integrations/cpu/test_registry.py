...
def test_registry_structure_sanity():
    ...
    for model in expected_models:
        ...
        assert registry[model].startswith("sklearn."), f"{model} should map to sklearn class"
...
def test_registry_model_paths_sanity():
    ...
    for model_name, class_path in registry.items():
        if model_name == "xgboost":
            assert class_path.startswith("xgboost."), f"{model_name} path should start with xgboost."
        else:
            assert class_path.startswith("sklearn."), f"{model_name} path should start with sklearn."
...
def test_registry_xgboost_integration_unit():
    ...
    if "xgboost" in registry:
        assert registry["xgboost"] == "xgboost.XGBClassifier", "XGBoost should map to correct class"
    else:
