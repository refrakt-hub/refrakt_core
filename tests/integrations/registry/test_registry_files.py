...
SKLEARN_REGISTRY_PATH = PROJECT_ROOT / "src" / "refrakt_core" / "integrations" / "registry" / "sklearn_registry.yaml"
CUML_REGISTRY_PATH = PROJECT_ROOT / "src" / "refrakt_core" / "integrations" / "registry" / "cuml_registry.yaml"
...
def test_sklearn_registry_expected_models_sanity():
    ...
    for model in expected_models:
        ...
        assert content[model].startswith("sklearn."), f"{model} should map to sklearn class"
...
def test_registry_key_uniqueness_unit():
    ...
    for registry_path in registry_paths:
        ...
        assert len(keys) == len(unique_keys), f"Registry keys should be unique in {registry_path.name}"
...
def test_registry_value_uniqueness_unit():
    ...
    for registry_path in registry_paths:
        ...
        assert len(values) == len(unique_values), f"Registry values should be unique in {registry_path.name}"
...
def test_sklearn_registry_specific_mappings_unit():
    ...
    for model, expected_path in expected_mappings.items():
        if model in content:
            assert content[model] == expected_path, f"{model} should map to {expected_path}"
...
def test_registry_yaml_syntax_unit():
    ...
    for registry_path in registry_paths:
        try:
            with open(registry_path, 'r') as f:
                content = yaml.safe_load(f)
            assert content is not None, f"YAML should not be None for {registry_path.name}"
...
def test_registry_consistency_between_files_unit():
    ...
    assert common_models.issubset(sklearn_keys), "sklearn registry should contain common models"
    assert common_models.issubset(cuml_keys), "cuml registry should contain common models" 