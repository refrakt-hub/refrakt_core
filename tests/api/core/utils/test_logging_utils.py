...
class TestLoggingUtils:
    ...
    def test_logging_utils_has_any_function(self):
        funcs = [f for f in dir(logging_utils) if callable(getattr(logging_utils, f)) and not f.startswith('__')]
        assert funcs
    ...
    def test_extract_tensor_from_model_output_reconstruction(self):
        t = torch.ones(2, 2)
        out = logging_utils.extract_tensor_from_model_output(DummyOutput(reconstruction=t))
        assert out is not None and torch.equal(out, t)
    ...
    def test_handle_scalar_value(self):
        ...
        assert logging_utils._handle_scalar_value('b', torch.tensor([2.0])) == {'b': 2.0}
