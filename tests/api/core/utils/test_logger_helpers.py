...
class TestLoggerHelpers:
    ...
    def test_logger_helpers_has_any_function(self):
        funcs = [f for f in dir(logger_helpers) if callable(getattr(logger_helpers, f)) and not f.startswith('__')]
        assert funcs
    ...
    def test_create_metrics_to_log(self):
        ...
        prefix = 'train'
        logged_metrics = set()
        out = logger_helpers._create_metrics_to_log(metrics, step, prefix, logged_metrics)
        assert 'acc' in out and 'loss' in out
        # Duplicates should be filtered
        out2 = logger_helpers._create_metrics_to_log(metrics, step, prefix, logged_metrics)
        assert out2 == {}
