...
from refrakt_core.api.core import extras as utils
...
@patch("refrakt_core.api.core.extras.build_dataset")
def test_build_datasets_sanity(mock_build_dataset):
    # Create proper OmegaConf object
    cfg = OmegaConf.create({"dataset": {"params": {"train": True}}})
    
    mock_build_dataset.side_effect = lambda cfg: cfg
    train, val = utils.build_datasets(cfg)  # type: ignore
    assert train["params"]["train"] is True
    assert val["params"]["train"] is False


@patch("refrakt_core.api.core.extras.build_dataloader")
def test_build_dataloaders_sanity(mock_build_dataloader):
    ...
    train_loader, val_loader = utils.build_dataloaders(
        "train_ds", "val_ds", cfg  # type: ignore
    )
