...
def patch_dataset_registry(monkeypatch):
    ...
    monkeypatch.setattr('src.refrakt_core.registry.dataset_registry.get_dataset', lambda name, **params: DummyDataset(**params))
    yield
...
def patch_transform_registry(monkeypatch):
    ...
    monkeypatch.setattr('src.refrakt_core.registry.transform_registry.get_transform', lambda name, *args, **kwargs: DummyTransform(**kwargs))
    monkeypatch.setattr('src.refrakt_core.api.builders.transform_builder.build_transform', lambda cfg: lambda x: x)
    yield
...
class TestDatasetBuilder:
    ...
    def test_build_dataset_with_wrapper_smoke(self, base_cfg, monkeypatch):
        ...
        monkeypatch.setitem(__import__('src.refrakt_core.api.builders.dataset_builder', fromlist=['DATASET_REGISTRY']).__dict__['DATASET_REGISTRY'], 'wrapper', DummyWrapper)
        monkeypatch.setattr('src.refrakt_core.api.builders.dataset_builder.get_dataset', lambda name, **params: DummyDataset(**params))
        ds = build_dataset(base_cfg)
