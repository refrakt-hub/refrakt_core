...
@pytest.fixture
def base_cfg():
    return OmegaConf.create({'params': {'batch_size': 2, 'shuffle': True, 'num_workers': 0, 'drop_last': False}})
...
class TestDataloaderBuilder:
    ...
    def test_build_dataloader_sanity_iterate(self, dataset, base_cfg):
        loader = build_dataloader(dataset, base_cfg)
        data = list(loader)
        assert all(isinstance(x, int) or (isinstance(x, torch.Tensor) and x.dtype == torch.int64) for batch in data for x in (batch if isinstance(batch, (list, tuple)) else [batch]))
