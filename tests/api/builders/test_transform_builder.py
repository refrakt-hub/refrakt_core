...
def test_build_transform_simple():
    ...
    with patch('refrakt_core.api.builders.transform_builder.get_transform') as get_transform:
...
def test_build_transform_nested():
    cfg = [
        {'name': 'RandomApply', 'params': {'transforms': [{'name': 'RandomHorizontalFlip', 'params': {}}], 'p': 0.5}}
    ]
    with patch('refrakt_core.api.builders.transform_builder.get_transform') as get_transform:
