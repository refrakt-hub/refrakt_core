...
class TestTransformBuilder:
    ...
    def test_build_transform_unit_randomapply(self, monkeypatch):
        ...
        cfg = [{'name': 'RandomApply', 'params': {'transforms': [{'name': 'dummy', 'params': {}}], 'p': 1.0}}]
        t = build_transform(cfg)
        assert callable(t)
    ...
    def test_build_transform_unit_pairedtransform(self, monkeypatch):
        ...
        class MockPairedTransform:
            ...
            def __call__(self, x):
                return x
        monkeypatch.setattr('src.refrakt_core.transforms.PairedTransform', MockPairedTransform)
        cfg = [{'name': 'PairedTransform', 'params': {'transforms': [{'name': 'dummy', 'params': {}}]}}]
        t = build_transform(cfg)
        assert callable(t)
