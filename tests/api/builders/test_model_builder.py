...
class TestModelBuilder:
    ...
    def test_build_model_unit_fusion_block(self, base_cfg, modules, monkeypatch):
        ...
        monkeypatch.setattr(model_utils, 'add_fusion_block', lambda model, cfg, device: (called.setdefault('fusion', True) or model))
    ...
    def test_build_model_unit_apply_model_overrides(self, base_cfg, modules, monkeypatch):
        monkeypatch.setattr('src.refrakt_core.api.builders.utils.model_utils.apply_model_overrides', lambda cfg, overrides: base_cfg)
        model = build_model(base_cfg, modules, device='cpu', overrides=['model.params.foo=99'])
        assert isinstance(model, DefaultModelWrapper) or hasattr(model, 'model')

    def test_build_model_unit_validate_model_config_type_error(self, base_cfg, modules, monkeypatch):
        monkeypatch.setattr('src.refrakt_core.api.builders.utils.model_utils.validate_model_config', lambda cfg: (_ for _ in ()).throw(TypeError('fail')))
        build_model(base_cfg, modules, device='cpu')
