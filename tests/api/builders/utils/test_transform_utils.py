...
class TestTransformUtils:
    ...
    def test_build_simple_transform(self, monkeypatch):
        monkeypatch.setattr('src.refrakt_core.registry.transform_registry.get_transform', dummy_get_transform)
        t = transform_utils._build_simple_transform('dummy', {})
        assert callable(t)

    def test_build_nested_transform(self, monkeypatch):
        monkeypatch.setattr('src.refrakt_core.registry.transform_registry.get_transform', dummy_get_transform)
        t = transform_utils._build_nested_transform('RandomApply', {'transforms': [{'name': 'dummy', 'params': {}}], 'p': 1.0}, lambda cfgs: [DummyTransform() for _ in cfgs])
        assert callable(t)

    def test_build_transform_list(self, monkeypatch):
        monkeypatch.setattr('src.refrakt_core.registry.transform_registry.get_transform', dummy_get_transform)
        seq = [{'name': 'dummy', 'params': {}}]
        out = transform_utils._build_transform_list(seq, lambda cfgs: [DummyTransform() for _ in cfgs])
        assert isinstance(out, list)
