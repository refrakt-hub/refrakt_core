...
import refrakt_core.api.builders.utils.optimizer_utils as optimizer_utils
...
class TestOptimizerUtils:
    ...
    def test_build_component_optimizer(self):
        ...
        model = DummyGAN()
        opt = optimizer_utils.build_component_optimizer(
            comp_cfg, model, "generator", opt_map
        )
    ...
    def test_build_multi_component_optimizer(self):
        ...
        with pytest.raises(ValueError):
            optimizer_utils.build_multi_component_optimizer(
                optimizer_cfg, model, opt_map
            )
