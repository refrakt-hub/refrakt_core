...
class TestTrain:
    ...
    def test_train_sanity_success(self, monkeypatch, dummy_cfg):
        monkeypatch.setattr(train_mod, '_load_and_validate_config', lambda cfg: dummy_cfg)
        monkeypatch.setattr(train_mod, '_resolve_model_name_train', lambda cfg: 'dummy')
        monkeypatch.setattr(train_mod, '_setup_logging', lambda cfg, name, logger: DummyLogger())
        ...
        monkeypatch.setattr(train_mod, '_build_datasets_and_model', lambda cfg, modules, device, logger: ('train_loader', 'val_loader', 'model', 'loss_fn'))
        monkeypatch.setattr(train_mod, '_setup_optimizer_and_scheduler', lambda cfg, model: ('optimizer', 'scheduler'))
        monkeypatch.setattr('refrakt_core.api.utils.train_utils.setup_artifact_dumper', lambda cfg, name, logger: 'artifact_dumper')
        monkeypatch.setattr(train_mod, '_setup_trainer', lambda cfg, model, train_loader, val_loader, loss_fn, optimizer, scheduler, device, modules, artifact_dumper, name, logger: ('trainer', 10, 'cpu'))
        monkeypatch.setattr(train_mod, '_execute_training', lambda trainer, num_epochs, config, model, train_loader, val_loader, final_device, artifact_dumper, resolved_model_name, logger: {'trained': True})
    ...
    def test_train_unit_pure_ml(self, monkeypatch, dummy_cfg):
        ...
        monkeypatch.setattr(train_mod, '_handle_pure_ml_training', lambda cfg, name, logger: None)
        result = train_mod.train(cfg=dummy_cfg, logger=None)
        assert result == {'status': 'completed', 'type': 'ml'}

    def test_train_unit_error_handling(self, monkeypatch, dummy_cfg):
        monkeypatch.setattr(train_mod, '_load_and_validate_config', lambda cfg: (_ for _ in ()).throw(Exception('fail')))
        logger = DummyLogger()
        monkeypatch.setattr(train_mod, '_setup_logging', lambda cfg, name, logger_arg: logger)
    ...
    def test_train_unit_invalid_cfg(self, monkeypatch):
        monkeypatch.setattr(train_mod, '_load_and_validate_config', lambda cfg: (_ for _ in ()).throw(ValueError('bad cfg')))
        logger = DummyLogger()
        monkeypatch.setattr(train_mod, '_setup_logging', lambda cfg, name, logger_arg: logger)
        with pytest.raises(SystemExit):
    ...
    def test_train_unit_trainer_setup_called(self, monkeypatch, dummy_cfg):
        ...
        monkeypatch.setattr('refrakt_core.api.utils.train_utils.setup_artifact_dumper', lambda cfg, name, logger: 'artifact_dumper')
        called = {}
        def fake_setup_trainer(cfg, model, train_loader, val_loader, loss_fn, optimizer, scheduler, device, modules, artifact_dumper, name, logger):
            called['trainer'] = True
    ...
    def test_train_unit_execute_training_called(self, monkeypatch, dummy_cfg):
        ...
        monkeypatch.setattr(train_mod, '_setup_trainer', lambda cfg, model, train_loader, val_loader, loss_fn, optimizer, scheduler, device, modules, artifact_dumper, name, logger: ('trainer', 10, 'cpu'))
        called = {}
        def fake_execute_training(trainer, num_epochs, config, model, train_loader, val_loader, final_device, artifact_dumper, resolved_model_name, logger):
