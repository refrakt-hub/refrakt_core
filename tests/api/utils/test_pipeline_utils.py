...
class TestPipelineUtils:
    ...
    def test_setup_logger_and_config_success(self):
        ...
        logger = pipeline_utils.setup_logger_and_config(cfg, "resnet", "./logs", ["file"], True, False, ["foo=1"])
        assert type(logger).__name__ == "RefraktLogger"

    def test_setup_logger_and_config_typeerror(self):
        with pytest.raises(TypeError):
            pipeline_utils.setup_logger_and_config(42, "resnet", "./logs", ["file"], True, False, [])

    def test_setup_logger_and_config_valueerror(self):
        cfg = DictConfig({"model": {"name": "resnet"}})
        with pytest.raises(ValueError):
            pipeline_utils.setup_logger_and_config(cfg, "", "./logs", ["file"], True, False, [])
        with pytest.raises(ValueError):
            pipeline_utils.setup_logger_and_config(cfg, "resnet", "", ["file"], True, False, [])

    def test_execute_training_pipeline(self, monkeypatch, tmp_path):
        called = {}
        monkeypatch.setattr(pipeline_utils, "train", lambda *a, **kw: called.setdefault("train", True))
    ...
    def test_execute_testing_pipeline(self, monkeypatch, tmp_path):
        called = {}
        monkeypatch.setattr(pipeline_utils, "test", lambda *a, **kw: called.setdefault("test", True))
        monkeypatch.setattr("pathlib.Path.exists", lambda self: True)
        monkeypatch.setattr("os.path.exists", lambda path: True)
        monkeypatch.setattr(os, "makedirs", lambda *a, **kw: None)
        @contextmanager
    ...
    def test_execute_inference_pipeline(self, monkeypatch, tmp_path):
        called = {}
        monkeypatch.setattr(pipeline_utils, "inference", lambda *a, **kw: called.setdefault("inference", True))
        monkeypatch.setattr("pathlib.Path.exists", lambda self: True)
        monkeypatch.setattr("os.path.exists", lambda path: True)
        monkeypatch.setattr(os, "makedirs", lambda *a, **kw: None)
        @contextmanager
    ...
    def test_execute_full_pipeline(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(pipeline_utils, "train", lambda *a, **kw: called.update({"train": True}))
        monkeypatch.setattr(pipeline_utils, "test", lambda *a, **kw: called.update({"test": True}))
        monkeypatch.setattr(pipeline_utils, "inference", lambda *a, **kw: called.update({"inference": True}))
        monkeypatch.setattr("pathlib.Path.exists", lambda self: True)
        monkeypatch.setattr("os.path.exists", lambda path: True)
        monkeypatch.setattr(os, "makedirs", lambda *a, **kw: None)
        @contextmanager
