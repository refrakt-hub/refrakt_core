...
class TestTestEntrypoint:
    ...
    def test_test_sanity_success(self, monkeypatch, dummy_cfg):
        monkeypatch.setattr(
            test_mod, "_load_and_validate_config", lambda cfg: dummy_cfg
        )
        monkeypatch.setattr(test_mod, "_resolve_model_name", lambda cfg: "dummy")
        monkeypatch.setattr(
            test_mod, "_setup_logging", lambda cfg, name, logger: DummyLogger()
        )
        ...
        monkeypatch.setattr(
            ...
            lambda cfg, modules, device, logger: ("dataloader", "model", "loss_fn"),
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            ...
            lambda model, model_path, device, logger: None,
        )
        monkeypatch.setattr(
            test_mod,
            "_setup_fusion_evaluation",
            lambda cfg, model, dataloader, device, artifact_dumper, logger: "fusion_acc",
        )
        monkeypatch.setattr(
            test_mod,
            "_evaluate_model",
        ...
        logger = DummyLogger()
        monkeypatch.setattr(
            test_mod, "_setup_logging", lambda cfg, name, logger_arg: logger
        )
    ...
    def test_test_unit_pure_ml(self, monkeypatch, dummy_cfg):
        ...
        monkeypatch.setattr(
            test_mod, "_handle_pure_ml_pipeline", fake_handle_pure_ml_pipeline
        )
        test_mod.test(cfg=dummy_cfg, model_path="dummy.pth", logger=None)
