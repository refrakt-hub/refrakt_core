...
def patch_model_registry(monkeypatch):
    ...
    monkeypatch.setattr('src.refrakt_core.registry.model_registry.get_model', lambda name, *args, **kwargs: DummyModel(**kwargs))
    yield
...
class TestInferenceHelpers:
    ...
    def test_load_and_validate_config_calls_load_config(self, monkeypatch):
        from omegaconf import DictConfig
        called = {}
        monkeypatch.setattr('src.refrakt_core.api.helpers.inference_helpers.load_config', lambda cfg: called.setdefault('load', True))
        monkeypatch.setattr('src.refrakt_core.api.helpers.inference_helpers.OmegaConf.load', staticmethod(lambda x: DictConfig({'dummy': True})))
    ...
    def test_setup_logging_creates_logger(self, monkeypatch):
        ...
        monkeypatch.setattr("src.refrakt_core.api.helpers.inference_helpers.OmegaConf", type("OmegaConf", (), {"to_container": staticmethod(lambda cfg, resolve=True: {"foo": 1})}))
    ...
    def test_setup_data_loader_calls_resize(self, monkeypatch):
        ...
        def mock_resize(config, data, logger):
            called["resize"] = True
            return "loader"
        monkeypatch.setattr("src.refrakt_core.api.utils.train_utils.setup_data_loader_for_inference_with_resize", mock_resize)
