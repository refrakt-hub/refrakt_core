...
def patch_pipeline_utils(monkeypatch):
    ...
    monkeypatch.setattr("src.refrakt_core.api.utils.pipeline_utils.execute_training_pipeline", lambda *a, **kw: called.setdefault("train_util", True))
    monkeypatch.setattr("src.refrakt_core.api.utils.pipeline_utils.execute_testing_pipeline", lambda *a, **kw: called.setdefault("test_util", True))
    monkeypatch.setattr("src.refrakt_core.api.utils.pipeline_utils.execute_inference_pipeline", lambda *a, **kw: called.setdefault("inference_util", True))
    monkeypatch.setattr("src.refrakt_core.api.utils.pipeline_utils.execute_full_pipeline", lambda *a, **kw: called.setdefault("pipeline_util", True))
    yield called
...
def patch_dataset_registry(monkeypatch):
    ...
    monkeypatch.setattr("src.refrakt_core.registry.dataset_registry.get_dataset", mock_get_dataset)
    monkeypatch.setattr("src.refrakt_core.registry.dataset_registry.DATASET_REGISTRY", {
        "dummy": DummyBaseDataset,
        "contrastive": ContrastiveDataset
    })
    monkeypatch.setattr("src.refrakt_core.registry.transform_registry.get_transform", mock_get_transform)
    monkeypatch.setattr("src.refrakt_core.registry.transform_registry.TRANSFORM_REGISTRY", {
...
class TestCLIHelpers:
    ...
    def test_extract_overrides_combines_flags_and_positional(self, monkeypatch):
        ...
        monkeypatch.setattr("refrakt_core.hooks.hyperparameter_override.extract_overrides_from_args", lambda rem: (["baz=3"], []))
    ...
    def test_apply_config_overrides_applies_overrides(self, monkeypatch):
        ...
        monkeypatch.setattr("refrakt_core.hooks.hyperparameter_override.apply_overrides", lambda cfg, overrides: {"dataloader": {"params": {"batch_size": 64}}})
        cfg = DictConfig({"dataloader": {"params": {"batch_size": 32}}})
        out = cli_helpers._apply_config_overrides(cfg, ["dataloader.params.batch_size=64"])
    ...
    def test_setup_logging_config_with_args(self):
        runtime_cfg = {"mode": "test", "log_dir": "/tmp", "log_type": "file", "console": False, "model_path": "foo.pth", "debug": True}
        result = cli_helpers._setup_logging_config(runtime_cfg, args_log_dir="/override")
