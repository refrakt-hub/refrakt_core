...
class TestMainEntrypoint:
        ...
        def fake_setup_argument_parser():
            class DummyParser:
                def parse_known_args(self):
                    ...
                    return DummyArgs(), []
            return DummyParser()
        monkeypatch.setattr(__main__, '_setup_argument_parser', fake_setup_argument_parser)
        monkeypatch.setattr(__main__, '_extract_overrides', lambda args, rem: [])
        monkeypatch.setattr(__main__, 'OmegaConf', type('OmegaConf', (), {
            'load': staticmethod(lambda x: DictConfig({'model': {'name': 'dummy'}})),
            'is_config': staticmethod(lambda x: True)
        }))
        monkeypatch.setattr(__main__, '_apply_config_overrides', lambda cfg, overrides: cfg)
        monkeypatch.setattr(__main__, '_extract_runtime_config', lambda cfg: cfg)
        monkeypatch.setattr(__main__, '_setup_logging_config', lambda cfg, log_dir: ('train', 'log_dir', [], True, None, False))
        monkeypatch.setattr(__main__, 'setup_logger_and_config', lambda *a, **kw: DummyLogger())
    ...
    def test_main_pipeline_mode_dispatch(self, monkeypatch):
        ...
        def fake_execute_pipeline_mode(mode, cfg, model_path, logger):
            called['mode'] = mode
        monkeypatch.setattr(__main__, '_setup_argument_parser', lambda: type('DummyParser', (), {'parse_known_args': staticmethod(lambda: (DummyArgs(), []))})())
        monkeypatch.setattr(__main__, '_extract_overrides', lambda args, rem: [])
        monkeypatch.setattr(__main__, 'OmegaConf', type('OmegaConf', (), {
            'load': staticmethod(lambda x: DictConfig({'model': {'name': 'dummy'}})),
            'is_config': staticmethod(lambda x: True)
        ...
        monkeypatch.setattr(__main__, '_apply_config_overrides', lambda cfg, overrides: cfg)
        monkeypatch.setattr(__main__, '_extract_runtime_config', lambda cfg: cfg)
        monkeypatch.setattr(__main__, '_setup_logging_config', lambda cfg, log_dir: ('test', 'log_dir', [], True, None, False))
        monkeypatch.setattr(__main__, 'setup_logger_and_config', lambda *a, **kw: DummyLogger())
        monkeypatch.setattr(__main__, '_execute_pipeline_mode', fake_execute_pipeline_mode)
        __main__.main()
    ...
    def test_main_logger_finalization(self, monkeypatch):
        ...
        monkeypatch.setattr(__main__, '_extract_runtime_config', lambda cfg: cfg)
        monkeypatch.setattr(__main__, '_setup_logging_config', lambda cfg, log_dir: ('train', 'log_dir', [], True, None, False))
        monkeypatch.setattr(__main__, 'setup_logger_and_config', fake_setup_logger_and_config)
        monkeypatch.setattr(__main__, '_execute_pipeline_mode', lambda *a, **kw: None)
        __main__.main()
    ...
    def test_main_keyboard_interrupt(self, monkeypatch):
        ...
        monkeypatch.setattr(__main__, '_extract_runtime_config', lambda cfg: cfg)
        monkeypatch.setattr(__main__, '_setup_logging_config', lambda cfg, log_dir: ('train', 'log_dir', [], True, None, False))
        monkeypatch.setattr(__main__, 'setup_logger_and_config', lambda *a, **kw: logger)
        monkeypatch.setattr(__main__, '_execute_pipeline_mode', fake_execute_pipeline_mode)
        __main__.main()
